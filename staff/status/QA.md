...
| PR | Branch | Role | Что внутри |
|---|---|---|---|
| #43 | `m5/art` | Artist | 3 boss sprites 128×128 + 3 boss-drop icons 64×64 + 3 T3 icons 64×64 + gas overlay + gen_m5_assets.py; +69 KB / 412 KB total |
| #44 | `m5/content` | Content Designer | mobs.json (8→11), items.json (29→35), recipes.json (15→18), zones.json (boss_id, gas, daily, warehouse depth 3) |
| #45 | `m5/world` | Engineer | boss AI 2-phase, daily instance, gas zones, T3 craft, MobRole runtime, overkill queue, +20 vitest (128→148) |

## Octopus-merge

**Статус: FAILED — BLOCKER.**

```
git checkout -b qa/m5-acceptance-test m5-integration
git fetch origin m5/art m5/content m5/world
git merge --no-ff origin/m5/art origin/m5/content origin/m5/world
```

Результат: 5 file conflicts — octopus merge не прошёл.

| # | Conflicted file | Конфликт между |
|---|---|---|
| 1 | `src/systems/__tests__/dailyInstance.test.ts` | m5/art vs m5/world (оба добавили differently) |
| 2 | `src/systems/__tests__/gasZone.test.ts` | m5/art vs m5/world (оба добавили differently) |
| 3 | `src/systems/mobAI.ts` | m5/art vs m5/world (оба модифицировали) |
| 4 | `src/types/mob.ts` | m5/art vs m5/world (оба модифицировали) |
| 5 | `src/types/zone.ts` | m5/art vs m5/world (оба модифицировали) |

**Root cause:** m5/art branch содержит content + engineer commits (не только artist work). m5/world branch также содержит content/*.json modifications. Cross-PR overlap — роли не соблюдали strict file boundaries.

**Resolution:** Per M3+M4 lesson — **NOT resolving independently**. Escalating to PM / role owners. Role owners must create fix PRs that:
1. Artist: remove content/engineer commits from m5/art (rebase onto m5-integration, keep only assets/ + tools/ + staff/status/ARTIST.md)
2. Engineer: remove content/*.json modifications from m5/world (or accept content branch version)
3. Re-run octopus-merge after fixes

## Gate 1: Static check

**Статус: PASS** (на ветке `m5/world`, HEAD `e70a940`)

> NOTE: Cannot test on octopus-merged result due to conflict. Numbers below are from engineer branch only.

| Check | Result | Expected | Status |
|---|---|---|---|
| `npm run typecheck` | 0 errors | clean | **PASS** |
| `npm run lint` | 0 errors/warnings | clean | **PASS** |
| `npm run test` | **148** passed, 0 failed | ≥148, 0 fail | **PASS** |
| `npm run build` | **1.48 MB** (1,519.73 KB dist) | ≤ 2 MB | **PASS** |
| `du -sk assets/` | **324 KB** | ≤ 600 KB | **PASS** |

Exact vitest breakdown:
- gasZone.test.ts: 3 tests
- dailyInstance.test.ts: 3 tests
- radio.test.ts: 9 tests
- zoneUnlock.test.ts: 11 tests
- perks.test.ts: 15 tests
- xp.test.ts: 27 tests
- mobRole.test.ts: 3 tests
- mobAI.test.ts: 21 tests
- craft.test.ts: 13 tests
- loot.test.ts: 7 tests
- combat.test.ts: 18 tests
- weight.test.ts: 18 tests
- **Total: 148 tests, 12 files, 734ms**

## Gate 2: Runtime smoke

**Статус: PARTIAL PASS** (code review only, no browser available)

Code review confirms all M5 features are implemented in `m5/world`:

### M2 regression
- BaseScene → MapScene → Forest → SortieScene → CombatScene → LootScene → ReturnScene → CraftScene → InventoryScene flow: all scenes present in main.ts registry. **PASS (code review)**

### M3 regression
- MapScene: 3 zones visible. RadioScene: UI-stub remains. Zone multipliers: 1.0/1.2/1.5. **PASS (code review)**

### M4 regression
- CombatScene: XP gain → gainXP → levelled_up → LevelUpScene launch. ProgressionScene exists. 8 perks + veteran_conditioning fallback. **PASS (code review)**

### M5 boss-fight
- `initBossFight(mob)` checks `mob.role === "boss"`, creates `MobRuntimeState` with `phase: 1`. **PASS**
- CombatScene: boss HUD overlay `Босс: <name>` + `Фаза ${phase}` label. **PASS**
- `computePhaseTransition`: uses `mob.phase_threshold ?? 0.5`, triggers phase 2 when `hp/hp_max < threshold`, sets `phase_2_behavior_id`. **PASS**
- `chooseMobActionV2`: checks phase transition before action selection, swaps to `effectiveBehaviorId`. **PASS**
- Phase 2 popup: `inst.state.phase_transition_done && phase === 2 && !_phase2PopupShown` → log message. **PASS**
- Boss guaranteed drops: `getBossGuaranteedDrops` filters `drop_table` for `chance >= 1.0`. **PASS**

### M5 daily instance
- `canEnterDailyInstance(progress, zone, now)`: checks `zone.boss_id` exists, `daily_completed[zone.id]` cooldown. **PASS**
- `markDailyCompleted(progress, zoneId, now)`: sets timestamp. **PASS**
- MapScene: daily button visible (code in `m5/art` branch MapScene). **PASS (code review)**

### M5 gas zones
- `computeGasDamage(zone, depth, player)`: checks `level?.is_gas`, then `playerHasGasMask(player)`. Returns `zone.gas_damage_per_turn ?? 0` without mask, 0 with mask. **PASS**
- `playerHasGasMask`: checks `equipped_armor_id === "gas_mask"` OR `gas_mask` in backpack. **PASS**

### M5 T3 craft
- CraftScene: shows recipes including T3 (gated by ingredient availability). **PASS (code review)**
- `src/systems/craft.ts`: `canCraft` checks all ingredients present. **PASS**

### M5 LevelUpScene overkill queue
- `computeOverkillPopups(levelBefore, levelAfter)` returns `levelAfter - levelBefore`. **PASS**
- LevelUpScene: `popupQueue = computeOverkillPopups(...)`, decrements on perk selection, restarts if `popupQueue > 0`. **PASS**

## Gate 3: Spec compliance

### 3a. Anti-scope grep

**Статус: PASS** (with 1 false-positive note)
...
|---|---|---|---|
| `src/scenes/ReturnScene.ts:83` | "cooldown" | Comment: `// M5: mark daily instance cooldown...` | OK — comment |
| `src/state/types.ts:13` | "cooldown" | Comment: `// M5 GDD §9: daily instance cooldown per zone...` | OK — comment |
| `src/systems/dailyInstance.ts:16` | "cooldownMs" | Runtime variable: `const cooldownMs = resetHours * MS_PER_HOUR` | **False positive** — daily instance 24h reset timer IS M5 scope, NOT anti-scope skill-tree/ability cooldown |
| `src/systems/dailyInstance.ts:17` | "cooldownMs" | Runtime: `return now - lastCompleted >= cooldownMs` | **False positive** — same |
| `src/systems/__tests__/dailyInstance.test.ts:33` | "cooldown" | Test description string | **False positive** — same |
| `src/systems/__tests__/dailyInstance.test.ts:41` | "cooldown" | Test description string | **False positive** — same |
| `src/types/zone.ts:24` | "cooldown" | Comment: `// M5 GDD §9: daily instance cooldown...` | OK — comment |

**Verdict:** No anti-scope violations. All `cooldown` hits refer to the **daily instance 24h reset timer** which IS M5 scope (balance §M5.6, GDD §9.4). No `skill_tree`, `skill_point`, `active_ability`, `yandex_sdk`, `leaderboard`, `cloud_save`, `pvp`, `multiplayer`, `cinematic`, `trust_level`, `radio_reward`, `module_weapon`, `weapon_module`, `attachment` runtime hits.

### 3b. DoD numbers

**Статус: PASS** (content branch `origin/m5/content`)

| Metric | Actual | Expected | Status |
|---|---|---|---|
| `content/mobs.json` total | **11** (8 regular + 3 boss) | 11 | **PASS** |
| `content/items.json` total | **35** (29 + 3 boss-drop + 3 T3) | 35 | **PASS** |
| `content/recipes.json` total | **18** (15 + 3 T3) | 18 | **PASS** |
| `content/zones.json` zones | **3** (all boss_id filled) | 3 | **PASS** |
| M5 assets | **10** files (3 boss + 3 boss-drop + 3 T3 + 1 gas overlay) | 10 | **PASS** |
| vitest | **148** | 148 | **PASS** |

Verified programmatically via `python3` parsing of content branch JSON.

### 3c. JSON cross-ref validation

**Статус: PASS** (content branch)

| Cross-ref | Result | Status |
|---|---|---|
| `mobs[].boss_drop_id` → `items[].id` | mutated_gland ✓, prime_circuit ✓, captain_insignia ✓ | **PASS** |
| `recipes[].ingredients[].item_id` → `items[].id` | All 18 recipes, all ingredients valid | **PASS** |
| `recipes[].result_id` → `items[].id` | All 18 recipes, all results valid | **PASS** |
| `zones[].boss_id` → `mobs[].id` with `role:"boss"` | forest_alpha_mutant ✓, warehouse_drone_prime ✓, city_guard_captain ✓ | **PASS** |

### 3d. Balance/content/code consistency

**Статус: PASS** (content branch + m5/world code)

| Check | Content | balance.md §M5 | Match |
|---|---|---|---|
| forest_alpha_mutant hp | 300 | 300 | **PASS** |
| warehouse_drone_prime hp | 350 | 350 | **PASS** |
| city_guard_captain hp | 400 | 400 | **PASS** |
| All 3 boss phase_threshold | 0.5 | 0.5 | **PASS** |
| forest_alpha_mutant phase_2 | pack_bonus_when_paired | pack_bonus_when_paired | **PASS** |
| warehouse_drone_prime phase_2 | defensive_cover | defensive_cover | **PASS** |
| city_guard_captain phase_2 | ranged_keep_distance | ranged_keep_distance | **PASS** |
| warehouse gas_damage_per_turn | 5 | 5 | **PASS** |
| city gas_damage_per_turn | 8 | 8 | **PASS** |
| forest daily_reset_hours | 24 | 24 | **PASS** |
| warehouse daily_reset_hours | 24 | 24 | **PASS** |
| city daily_reset_hours | 24 | 24 | **PASS** |
| composite_blade stats | dmg 24-32, speed 85, wt 3 | same | **PASS** |
| prime_shotgun stats | dmg 27-37, speed 65, wt 4 | same | **PASS** |
| captain_armor stats | def 12, wt 5 | same | **PASS** |
| is_gas depth 2-3 | warehouse ✓, city ✓ | §M5.5 | **PASS** |
| forest is_gas | undefined (no gas) | §M5.5 | **PASS** |
| `computePhaseTransition` uses `mob.phase_threshold` | `mob.phase_threshold ?? 0.5` (NOT hardcoded 0.5) | §M5.1 | **PASS** |
...
| Check | Result | Status |
|---|---|---|
| 8 existing mobs: role "regular" or undefined | All 8 regular mobs have no `role` field (implicit "regular") | **PASS** |
| 29 existing items unchanged (+6 added) | First 29 items identical to M4 baseline; 6 new items appended | **PASS** |
| 15 existing recipes unchanged (+3 added) | First 15 recipes identical to M4 baseline; 3 new T3 recipes appended | **PASS** |
| M1+M3+M4 assets not regenerated | `git diff origin/m5-integration...origin/m5/art -- assets/` shows only 10 new files, 0 modifications | **PASS** |

## Additional findings

1. **`boss_drop_id` not in TypeScript `Mob` interface** — content/mobs.json includes `boss_drop_id` on boss mobs, but `src/types/mob.ts` does not define this field. The code uses `drop_table.filter(d => d.chance >= 1.0)` for guaranteed drops instead. Not a functional bug (drops work via drop_table), but a schema gap between content and code. **_Non-blocking M6+ follow-up_**: add `boss_drop_id?: string` to Mob interface for type safety and cross-ref validation.

2. **Zone-level `is_gas` / `gas_damage_per_turn` not consumed by code** — content/zones.json has `is_gas: true` and `gas_damage_per_turn` at zone level AND at level depth. The code (`computeGasDamage`) only checks `level?.is_gas`, not the zone-level flag. Zone-level fields are documentation only. **_Non-blocking M6+ follow-up_**: align code to use zone-level gas fields, or remove zone-level duplicates.

## Verdict

**CHANGES_REQUESTED.**

### Blockers (must fix before PM merge sequence)

1. **Octopus-merge FAILED** — 5 file conflicts between m5/art, m5/content, m5/world. Root cause: m5/art branch contains content + engineer commits; m5/world branch contains content/*.json modifications. Role owners must create fix PRs:
   - Artist: rebase m5/art onto m5-integration, keep only assets/ + tools/ + staff/status/ARTIST.md changes
   - Engineer: remove content/*.json changes from m5/world (content branch is the authoritative source for JSON)
   - After fixes: re-run octopus-merge → must be clean before proceeding

### Non-blocking M6+ follow-ups

1. `boss_drop_id` not in TypeScript `Mob` interface — schema gap between content JSON and code types
2. `cooldown` variable name in `dailyInstance.ts` triggers anti-scope grep false positive — consider renaming to `resetDurationMs` or similar in M6+
3. Zone-level `is_gas`/`gas_damage_per_turn` fields not consumed by code — redundancy

## Exact numbers (DoD-precision)
...
| build size | **1.48 MB** |
| assets size | **324 KB** |
| mobs total | **11** (8 regular + 3 boss) |
| items total | **35** (29 + 3 boss-drop + 3 T3) |
| recipes total | **18** (15 + 3 T3) |
| zones total | **3** (all boss_id filled) |
| M5 assets | **10** files |

## Recovery

- Role: QA Acceptance Critic M5.
- Milestone: M5 — Боссы и инстансы — **CHANGES_REQUESTED**.
- Branch: `qa/m5-acceptance` (base `m5-integration`).
- Objects under review: PR #43 (`m5/art`), PR #44 (`m5/content`), PR #45 (`m5/world`).
- Blockers: octopus-merge conflict (5 files). Escalate to PM for role-owner fix PRs.
- Next concrete step: role owners create fix PRs → re-run octopus-merge → re-review.
- Forbidden: resolve cross-PR conflicts independently; self-merge; modify src/content/assets/docs.
