...
- **Активная веха:** M7 — Полировка и баланс. Gate: `M7_PREPARED_PENDING_KICKOFF_MERGE → GD_IN_PROGRESS` (PM создал `m7-integration` от `main` HEAD `859a652`, готовит PR `pm/m7-kickoff → m7-integration`: dashboard + 6 kickoff + 6 handoff + shared dashboards). Подробно — `staff/status/M7.md`.

**Merge-делегация на M7 (continuation M3+M4+M5+M6):** Alex явно делегировал PM gate-close merge в `main` на M3; M4, M5, M6 продолжили делегацию (gate-close PR #39, #47, #57 merged PM). M7 продолжает её же: PM сам мерджит role-PR в `m7-integration` после QA Acceptance APPROVE и gate-close PR `m7-integration → main`, если Alex не изменит политику явно.

- **Закрытая веха M6:** см. `staff/status/M6.md` и `staff/handoff/M6-SUMMARY.md` (gate-close PR #57 merged 2026-05-25 PM по делегации Alex'а; `main` HEAD `859a652`).
