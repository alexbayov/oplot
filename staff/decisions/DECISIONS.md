...
### 2026-05-25 — M8b: монетизационная модель предложена PM на основе Yandex SDK research
- **Решение:** M8b скоуп определён на основе полного исследования Yandex Games SDK monetization API (ads + IAP + banner). Rewarded: 4 триггера (×2 loot / second-chance-50%-HP / daily-reset / gas+1). Interstitial: 1 размещение (post-sortie). IAP: 3 продукта (disable_ads / starter_pack / gas_pack), client-side обработка (`signed: false`), mandatory unprocessed-check §1.13.1. Ads-remover: `disable_ads` → все rewarded становятся instant-reward, interstitial/banner отключены. Цены: ~99/49/29 YAN (предварительно). Devin sub-agents решено не использовать — PM берёт оркестрацию напрямую.
- **Кто:** PM (Alex-ассистент), на основе Yandex SDK docs (yandex.com/dev/games/doc/en/sdk).
- **Обоснование:** M8a закрыта — SDK singleton, cloud save, viewport, locale готовы. Монетизация — следующий логический шаг перед M9 (сабмит). Исследование API показало, что весь monetization surface компактен (3 API-семейства: ads.adv, payments, banner) и хорошо документирован. Client-side IAP достаточен для MVP — server-side верификация (`signed: true`) добавляет сложность без необходимости для survival-RPG с микроплатежами. 4 rewarded-триггера покрывают основной game-loop без раздражения игрока (ни в бою, ни в крафте).
