...
describe("M12.5 combat content contracts", () => {
  ...
  test("weapon-sensitive items expose non-empty display names and safe generic metadata when present", () => {
    const items = loadItems();
    loadContentItems(itemMap(items));
    ...
    for (const item of items) {
      ...
      const rawGeneric = item.name_generic_ru;
      if (typeof rawGeneric !== "string") continue;
      expect(rawGeneric.trim(), `${item.id} name_generic_ru`).not.toBe("");
      for (const marker of releaseSafeNameMarkers) {
        expect(rawGeneric, `${item.id} generic name should not expose ${marker}`).not.toContain(marker);
      }
    }
  });
});
