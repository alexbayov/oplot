...
import { clearRegistry, getItem, itemName, loadContentItems } from "../ItemRegistry";
...
describe("M12.5 combat content contracts", () => {
  ...
  test("weapon-sensitive items have display names and safe generic fallback data when configured", () => {
    const items = loadItems();
    loadContentItems(itemMap(items));
    ...
    for (const item of items) {
      ...
      expect(itemName(m11).trim(), `${item.id} current display name`).not.toBe("");
      if (typeof item.name_generic_ru !== "string") continue;
      expect(item.name_generic_ru.trim(), `${item.id} generic display name`).not.toBe("");
      for (const marker of releaseSafeNameMarkers) {
        expect(item.name_generic_ru, `${item.id} generic name should not expose ${marker}`).not.toContain(marker);
      }
    }
  });
});
