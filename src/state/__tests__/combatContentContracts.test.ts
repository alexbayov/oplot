...
describe("M12.5 combat content contracts", () => {
  ...
  test("default player-facing weapon-sensitive names remain generic and release-safe", () => {
    const items = loadItems();
    loadContentItems(itemMap(items));
    ...
    for (const item of items) {
      ...
      for (const marker of releaseSafeNameMarkers) {
        expect(displayName, `${item.id} should not expose ${marker}`).not.toContain(marker);
      }
    }
  });
});
