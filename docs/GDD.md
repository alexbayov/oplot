...
[SortieScene: выбор глубины 1 / 2 / 3]
    │ depth ≤ min_player_level требуется для глубины 2 и 3
    ▼
...
|---|---|---|---|
| `marauder` | Мародёр | `human` | Стандартный противник. При HP<30% переходит в `flee` (попытка убежать — в MVP трактуется как «пропускает ход и теряет 5% инициативы», в M3+ — реальный escape). |
...
interface ArmorStats {
  ...
  vs_melee_bonus?: number;       // доп. защита против melee-урона (cloth_jacket vs wild_dog)
}
