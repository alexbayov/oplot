...
> **⚠ M13 PIVOT (2026-06-13).** Жанр изменился: с «Survival RPG с пошаговым боем» на «post-apoc shelter sim с крафтом и вылазками». Тактический бой удалён в M13 PR-1, заменён авторесолвом с decision surface (`src/systems/sortieResolve.ts`). Описания старого боя, ammo/каверов/инициативы, M11.0+M12 архитектуры — отправлены в `docs/redesign/archive/m12/`. Действительный контракт по новому core loop, крафту и базе: `docs/redesign/M13-PIVOT.md`. Многие секции ниже описывают legacy MVP-флоу M1-M2 (когда бой ещё был тактическим). Не используй их как спеку — читай как исторический контекст. Возврат тактики — в M14+ отдельным треком.
>
